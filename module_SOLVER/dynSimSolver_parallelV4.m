...
if ~integration_flag % for debudding purposes
    1;
end
...
OMEGAe = X(:,13); OMEGAt = X(:,14);

%% 1. CONTROL VECTOR UPDATE (prescribed driver models)
c.costf = "notdefinedyet";
c = controlMap(c,T,c.costf);
...
p.gear(c.delta_c==1) = 0; % set gear to zero if clutch disengaged
% p.gear(any(abs(o.speed - p.shift_speeds(2:end))<0.5)) = 0; % workaround for gear shift - Doesn't work (not able to "cross" it
...
[DOMEGAe,DOMEGAt] = RotatoryEoMsolver(i,p,c,M,OMEGAe,OMEGAt);
